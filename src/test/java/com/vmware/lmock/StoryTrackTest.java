...
import static com.vmware.lmock.Dalton.*;
import static com.vmware.lmock.LMAsserts.*;
...
public class StoryTrackTest {
    ...
    public void testAnEmptyStoryTrackWithMultipleThreadsNotRunning() {
        Thread messenger1 = Messenger.anEmptyMessenger();
        Thread messenger2 = Messenger.anEmptyMessenger();
        Role[] roles = {
    ...
    public void testAnEmptyStoryTrackWithMultipleThreadsRunning() throws
      InterruptedException {
        Messenger messenger1 = Messenger.anEmptyMessenger();
        Messenger messenger2 = Messenger.anEmptyMessenger();
        Role[] roles = {
            new Role(anActorForThread(messenger1)),
            new Role(anActorForThread(messenger2))
        };
        begin(roles);
        Messenger.startMessengers(messenger1, messenger2);
        Messenger.stopMessengers();
        end();
    ...
    public void testAStoryWithASingleOccurrence() {
        ...
        assertTrue(
          track.contains(
          "satisfied 1 time: interface com.vmware.lmock.Dalton.ping():void/[1..1]"));
    ...
    public void testAStoryWithSingleOccurrenceOnOneThread() throws
      InterruptedException {
        ...
        Messenger messenger = Messenger.aMessenger(processor);
        Role role = new Role(anActorForThread(messenger));

        begin(role);
        role.willInvoke(1).of(joe).ping();
        Messenger.startMessengers(messenger);
